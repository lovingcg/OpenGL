...
// camera
Camera camera(glm::vec3(1.3f, 0.0f, 6.0f));
...
int main()
{
    ...
    // --------------------
    GLFWwindow* window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Normal Parallax Mapping", NULL, NULL);
    if (window == NULL)
    {
    ...
    Shader shader1("4.normal_mapping.vs", "4.normal_mapping.fs");

    // load textures
    // -------------
    unsigned int diffuseMap1 = loadTexture("resources/textures/bricks2.jpg");
    unsigned int normalMap1 = loadTexture("resources/textures/bricks2_normal.jpg");
    unsigned int heightMap1 = loadTexture("resources/textures/bricks2_disp.jpg");
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //model = glm::translate(model, glm::vec3(2.0f, 0.f, 0.f));
        ...
        // render parallax-mapped quad
        model = glm::mat4(1.0f);
        model = glm::translate(model,glm::vec3(3.0f,0.f,0.f));
        model = glm::rotate(model, glm::radians((float)glfwGetTime() * -10.0f), glm::normalize(glm::vec3(1.0, 0.0, 1.0))); // rotate the quad to show parallax mapping from multiple directions
